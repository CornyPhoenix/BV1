...
\newcommand{\twodo}[1]{\vspace{11pt}\textcolor{red}{\textbf{todo:} #1}}
...
\begin{document}
	...
	See \\
	\textbf{Image Processing, Analysis, and Machine Vision} \\
	by \\
	...
	\twodo{(Sibel @Nour) Thanks for doing this part.:) Could you maybe split the second paragraph in multiple sentences? That would be really great. I've also computed the result of the described convolution. Could you maybe relate what you expected the result to be with \Cref{fig:task_1d}?}
	
	\begin{figure*}
		\centering
		\begin{tabular}{c c}
			\includegraphics[width=\columnwidth]{figures/B1.png}
			&
			\includegraphics[width=\columnwidth]{figures/B2.png}\\
			B1 & B2
		\end{tabular}
		\caption{\textit{Left:} Image B1 showing a bright square before a darker background. \textit{Right:} Result of convolving B1 with itself. B2 was obtained by performing multiplying the Fourier transform of the image with itself in the frequency domain and inverting the result back to the spatial domain.}
		\label{fig:task_1d}
	\end{figure*}
	...
	See file \texttt{task\_2a.py}.
	
	\twodo{(Sibel) Apparently, this is not correct. I will go and talk to Benjamin to make sure whether I understood the task correctly.}
	
	\subsection{Fourier-Transform}
	
	See file \texttt{task\_2b.py}.
